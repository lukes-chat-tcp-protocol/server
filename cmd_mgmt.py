class handleToMode:
    def __init__(self, sock, addr, mode, comms):
        ...
        self.comms.send(conn, client_env, b'READY')
        while True:
            try:
                recv = self.comms.recv(conn, client_env, 1024).decode('utf-8')
            except UnicodeDecodeError:
                self.comms.send(conn, client_env, b'ERROR InvalidCommand')
            else:
                args = recv.split(' ')
        ...
        def check_login(permission_level):
            if client_env['login']:
                if client_env['permission_level'] >= permission_level:
                    ...
                    self.comms.send(conn, client_env, b'ERROR PermissionDenied')
            else:
                self.comms.send(conn, client_env, b'ERROR LoginRequired')
                return False
        if args[0] == 'CLOSE':
            return 'break'
        elif args[0] == 'LOGIN':
            try:
                ...
                self.comms.send(conn, client_env, b'ERROR InvalidB64Code')
            else:
                auth_attempt = auth.login(username, password)
                if auth_attempt == False:
                    self.comms.send(conn, client_env, b'ERROR LoginFailed')
                else:
                    self.comms.send(conn, client_env, b'ACK')
